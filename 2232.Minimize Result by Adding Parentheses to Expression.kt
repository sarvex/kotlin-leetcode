internal class Solution {
  fun minimizeResult(expression: String): String {
    val idx: Int = expression.indexOf('+')
    val l: String = expression.substring(0, idx)
    val r: String = expression.substring(idx + 1)
    val m = l.length
    val n = r.length
    ...
    for (i in 0 until m) {
      for (j in 0 until n) {
        val c: Int = l.substring(i).toInt() + r.substring(0, j + 1).toInt()
